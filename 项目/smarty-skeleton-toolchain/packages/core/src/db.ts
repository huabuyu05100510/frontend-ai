...
export async function getItem(key: string): Promise<string | null> {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.onsuccess = () => {
      ...
      if (req.result) resolve(decompressDSL(req.result) as any );
      else resolve(null);
    };
    req.onerror = () => reject(req.error);
  });
...
export function compressDSLBinaryExtreme(dsl: SkeletonDSL): Uint8Array {
  ...
  [
    ...
    "borderStyle",
    "borderColor",
  ].forEach((k) => pushUint16(out, (dict as any)[k].length));
  ...
  [
    ...
    "borderStyle",
    "borderColor",
  ].forEach((k) => (dict as any)[k].forEach(pushString));
...
function decompressDSLBinaryExtreme(
...
// -------------------- LZW 压缩 --------------------
function lzwEncode(u8: Uint8Array): Uint8Array {
...
function lzwDecode(u8: Uint8Array): Uint8Array {
  const codes: number[] = [];
...
// -------------------- 极致压缩 + LZW --------------------
function compressDSLBinaryUltra(dsl: SkeletonDSL): Uint8Array {
  return lzwEncode(compressDSLBinaryExtreme(dsl));
}

 function decompressDSLBinaryUltra(
  blob: Uint8Array | ArrayBuffer
): SkeletonDSL | null {
