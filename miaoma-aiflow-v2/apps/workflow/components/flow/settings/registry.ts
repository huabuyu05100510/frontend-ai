...
class NodeSettingsRegistry implements NodeSettingsStrategy {
    ...
    unregister(nodeType: NodeKind): boolean {
        return this.strategies.delete(nodeType)
    }
     ...
     * 获取所有已注册的插件节点类型
     */
    getPluginNodeTypes(): NodeKind[] {
        return Array.from(this.strategies.keys()).filter(t => t.startsWith('plugin:'))
    }

    /**
     * 检查是否是插件节点类型
     */
    isPluginNodeType(nodeType: string): boolean {
