 ...
 * 获取节点类型的标准输出变量
 */
function normalizeOutputType(type: unknown): NodeOutputVariable['type'] {
    return type === 'string' || type === 'number' || type === 'boolean' || type === 'array' || type === 'object' || type === 'any'
        ? type
        : 'any'
}

function normalizePluginOutputs(outputs: unknown): NodeOutputVariable[] {
    ...
    return outputs
        ...
        .map(output => {
            ...
            const name = typeof item.name === 'string' && item.name.trim() ? item.name : 'output'
...
export function getNodeTypeOutputs(nodeType: NodeKind, nodeData?: any): NodeOutputVariable[] {
    if (isPluginNodeType(nodeType)) {
        return normalizePluginOutputs(nodeData?.outputs)
    }
 ...
 * 获取节点类型的中文名称
 */
export function getNodeTypeLabel(nodeType: NodeKind, nodeData?: any): string {
    if (isPluginNodeType(nodeType)) {
        if (typeof nodeData?.label === 'string' && nodeData.label.trim()) {
            return nodeData.label
        }

        if (typeof nodeData?.pluginType === 'string' && nodeData.pluginType.trim()) {
            return nodeData.pluginType
        }

        const parsed = parsePluginNodeType(nodeType)
...
export function getAvailableNodeOutputs(currentNodeId: string, nodes: Node[], edges: Edge[]): AvailableNodeOutput[] {
    ...
    return upstreamNodes
        ...
        .map(node => ({
            ...
            nodeLabel: (node.data?.label as string) || getNodeTypeLabel(node.type as NodeKind, node.data),
