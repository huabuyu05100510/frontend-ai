# voice-kit · 简历包装版（语音领域技术代表作）

> 面向 14 年前端专家定位，按"项目卡片 + 技术深度弹药库"两层组织。
> 直接贴简历用「项目卡片」；面试展开用「弹药库」。
...
> 每条都是可独立深聊的技术点。面试官问到任一条，都能从「问题 → 方案 → 备选 → 权衡」四层展开。
...
1. `currentResponseId++`
2. 丢弃队列中所有 `chunk.responseId < current`
...
### 弹药 2 · chunk_id RTT + HDR Histogram
...
### 弹药 7 · 重连 + 会话恢复协议
**问题**：Socket.IO 默认重连是 5 次 ×1s 固定，弱网 5s 就放弃；重连后服务端会话丢失，音频被丢弃。

**方案**：
- 抖动指数退避：`base * 2^n * (0.5 + rand()*0.5)`，上限 30s
- `open` 后发 `{"type":"resume","token":T,"lastServerSeq":N}`；服务端从 N+1 回放
- 失败回退 fresh 会话，emit `resumed:false` 让场景决策（转写可接受；对话必须终结当前 response）

### 弹药 8 · 背压感知
**问题**：`socket.emit()` 在弱网下无限堆积 JS 内存，最终 OOM。

**方案**：监听 `engine.writeBuffer.length`，超 `highWaterBytes` 触发 `onPressure(true)`：
- 转写场景：捕获层切"环形丢弃"模式，保留最新 N ms，丢最老，自增 `dropped_frames{reason=backpressure}`
- 对话场景：静麦 + UI 警告（丢弃破坏语义）

### 弹药 9 · Speex WASM 抗混叠重采样
**问题**：48kHz→16kHz 用线性插值无抗混叠滤波，≥8kHz 频段折叠回来产生可听噪声，实测降低 ASR WER。

**方案**：集成 Speex WASM（Kaiser 窗多相 Sinc 滤波器），保相位；线性插值只在 `{allowAliasing:true}` 显式降级时用。

### 弹药 10 · Opus WASM 自适应码率
**问题**：16kHz 16-bit PCM = 256kb/s，4G 上行撑不住。

**方案**：Opus WASM 编码，码率按 `voice.transport.rtt_ms` p95 动态调（4G→8kb/s，WiFi→24kb/s），10 倍带宽节省且感知无损。

### 弹药 11 · Volcengine 二进制协议 TS 重写
**问题**：火山引擎 v3/sauc 协议无公开文档，原实现是 Python。

**方案**：凭抓包+Python 源码 TS 重写：
- 4 字节协议头（version + msg_type + flags + serialization/compression）
- gzip 压缩 JSON 负载
- flags bit0 决定 4 字节可选 seq 字段
- 三种帧编码器（握手/流式音频/终结）
- 黄金 fixture 测试：从真实会话抓的帧 → 解码 → 快照

### 弹药 12 · 卡拉OK 词级二分对齐
**问题**：流式 TTS 播放时实时高亮当前词，朴素遍历 O(n) 每帧触发 React 重渲染。

**方案**：
- `subtitleKaraoke.ts` 纯函数二分查找当前词索引（O(log n)）
- `useThrottledPartial` rAF 对齐节流（16ms leading + trailing），保证首帧即时 + 末帧必达
- Framer Motion `AnimatePresence` 词级过渡
